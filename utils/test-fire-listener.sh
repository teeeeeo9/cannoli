...
cast send "$liquidity_manager_rollup1_address" "mintNewLPPosition(address,address,uint256,address,address)" "$liq_hub_user_address" "$mock_pool_rollup1_address" "2" "$usdc_rollup1_address" "$usdt_rollup1_address"  --rpc-url "$rollup1_rpc_url" --private-key "$hub_ listener_private_key"
